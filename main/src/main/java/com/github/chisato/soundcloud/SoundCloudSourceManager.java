 ...
 */
public class SoundCloudSourceManager implements AudioSourceManager, HttpConfigurable {
    ...
    public static SoundCloudSourceManager createDefault() {
        ...
        return new SoundCloudSourceManager(true, dataReader, dataLoader, formatHandler, new DefaultSoundCloudPlaylistLoader(dataLoader, dataReader, formatHandler));
    }
     ...
     */
    public SoundCloudSourceManager(boolean allowSearch, SoundCloudDataReader dataReader, SoundCloudDataLoader dataLoader, SoundCloudFormatHandler formatHandler, SoundCloudPlaylistLoader playlistLoader) {
    ...
    private AudioItem loadSearchResult(String query, int offset, int rawLimit) {
        ...
        try (HttpInterface httpInterface = getHttpInterface(); CloseableHttpResponse response = httpInterface.execute(new HttpGet(buildSearchUri(query, offset, limit)))) {
    ...
    public static class Builder {
        ...
        public SoundCloudSourceManager build() {
            ...
            return new SoundCloudSourceManager(allowSearch, usedDataReader, usedDataLoader, usedFormatHandler, usedPlaylistLoader);
        }

        @FunctionalInterface
        interface PlaylistLoaderFactory {
            SoundCloudPlaylistLoader create(SoundCloudDataReader dataReader, SoundCloudDataLoader dataLoader, SoundCloudFormatHandler formatHandler);
        }
    }
}
